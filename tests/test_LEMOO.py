...
from XLEMOO.fitness_indicators import naive_sum
...
def init():
    ...
    ml_params = MLParams(0.3, 0.3, DecisionTreeClassifier(), 1.0, 0, 0, False, 1)
...
@pytest.mark.lemoo
def test_collect_population_no_unique(toy_model):
    toy_model.reset_generation_history()
    assert len(toy_model._generation_history) == 0
    ...
    ) = toy_model.collect_n_past_generations(past_n, unique_only=False)
